...
document.getElementById('btn-quit-test').addEventListener('click', async () => {
    ...
    document.getElementById('btn-start-learning').addEventListener('click', () => {
        ...
        const _cefr  = encodeURIComponent(localStorage.getItem('learner_cefr_level') || '');
            if (_cefr === 'A2') {
                window.location.href = `http://localhost:8000/homeA2/?learner_id=${_lid}&name=${_name}&email=${_email}&cefr_level=${encodeURIComponent(_cefr)}`;
            } else {
                window.location.href = `http://localhost:8000/?learner_id=${_lid}&name=${_name}&email=${_email}&cefr_level=${encodeURIComponent(_cefr)}`;
            }
    });
