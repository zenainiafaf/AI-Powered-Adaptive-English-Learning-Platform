"""
import_grammar_course_2.py
==========================
Importe le cours 2 — Word Order in English — dans la base PostgreSQL.

Placer ce script dans : PLATFORM/backend/
Placer le JSON dans   : PLATFORM/backend/grammar_a1_word_order.json

Lancer depuis PLATFORM/backend/ :
    python import_grammar_course_2.py
...
# python scripts\grammar\import_grammar_course_2.py
# ── Setup chemin Django ───────────────────────────────────────────────────────
...
COURSE_ORDER = {
    ...
    'grammar_a1_word_order'           : 2,   # ← cours 2
...
COURSE_CATEGORY = {
    ...
    'grammar_a1_word_order'           : 'phrases_noms_adjectifs',   # ← même catégorie
    # 'grammar_a1_numbers'              : 'chiffres_nombres',
    # 'grammar_a1_dates'                : 'chiffres_nombres',
}
...
if __name__ == '__main__':
    ...
    json_file = os.path.join(BASE_DIR, 'scripts', 'grammar', 'grammar_a1_word_order.json')

    # Accepte aussi un chemin passé en argument :
    # python import_grammar_course_2.py chemin/vers/mon_cours.json
    if len(sys.argv) > 1:
